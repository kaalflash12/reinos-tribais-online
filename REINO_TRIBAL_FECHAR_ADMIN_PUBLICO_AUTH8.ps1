...
$AuthCommit='a1bb9ded369345f7b1fae8bdc027051158b4b241'
$AuthFile='REINO_TRIBAL_ADMIN_FINAL_RT91_AUTH8.ps1'
...
$Tmp=Join-Path $env:TEMP ('reino-tribal-final-auth8-'+[Guid]::NewGuid().ToString('N'))
...
function Download([string]$Url,[string]$Out){
  ...
  try{$ErrorActionPreference='Continue';& $Curl --fail --silent --show-error --location --http1.1 --tlsv1.2 --retry 4 --retry-all-errors --connect-timeout 20 --max-time 120 --output $Out $Url 2>&1|Out-Null;$code=$LASTEXITCODE}finally{$ErrorActionPreference=$oldEap}
  if($code -ne 0 -or -not(Test-Path $Out) -or (Get-Item $Out).Length -eq 0){Fail ('Falha baixando '+$Url)}
}

try{
  New-Item -ItemType Directory -Force -Path $Tmp,$CredDir,$ProofDir|Out-Null
  Write-Host '=== REINOS TRIBAIS - FECHAMENTO TOTAL AUTH8 + ADM + MOBILE ===' -ForegroundColor Cyan
  Write-Host ('AUTH8 FINAL PINADO: '+$AuthCommit) -ForegroundColor DarkGray
  ...
  if($errors.Count){$errors|Format-List|Out-String|Write-Host;Fail 'AUTH8 FINAL nao passou no parser PowerShell.'}
  ...
  foreach($needle in @(
    ...
    'WINDOWS_POWERSHELL_NATIVE_STDERR_SAFE_CONTRACT',
    "@('deploy','orgs','list')",
    "@('deploy','whoami')",
  ...
  )){if(-not $authText.Contains($needle)){Fail ('AUTH8 FINAL sem contrato esperado: '+$needle)}}
  foreach($forbidden in @('console.deno.com/auth/interactive','console.deno.com/auth/exchange',"'deploy','--org'")){
    if($authText.Contains($forbidden)){Fail ('AUTH8 FINAL contem caminho proibido: '+$forbidden)}
  }
  Pass 'AUTH8 FINAL materializado, parseado e com stderr nativo protegido no PowerShell 5.1'

  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $AuthScript
  if($LASTEXITCODE -ne 0){Fail ('AUTH8 FINAL terminou com exit code '+$LASTEXITCODE)}
  if(-not(Test-Path $CredFile)){Fail ('Credencial final nao foi criada: '+$CredFile)}

  $cred=[IO.File]::ReadAllLines($CredFile)
  if(-not($cred -match '^Executor: RT91 / SAFE-CLI-AUTH-8-FINAL$')){Fail 'Credencial final nao confirma AUTH8 FINAL.'}
  ...
  Pass 'AUTH8 FINAL validou org/app + login + admin_status + dashboard sem imprimir senha'

  $Deno=Join-Path (Join-Path $env:LOCALAPPDATA 'ReinoTribalTools\deno-2.9.5') 'deno.exe'
  if(-not(Test-Path $Deno)){$dc=Get-Command deno.exe -ErrorAction SilentlyContinue;if($dc){$Deno=$dc.Source}else{Fail 'Deno 2.9.5 nao encontrado apos AUTH8.'}}
  ...
  try{
    ...
    try{
      ...
      & $Deno run --allow-all $BrowserTest 2>&1 | ForEach-Object { Write-Host ([string]$_) }
      $browserCode=$LASTEXITCODE
    }finally{$ErrorActionPreference=$oldEap}
    if($browserCode -ne 0){Fail ('Prova publica do navegador falhou com exit code '+$browserCode)}
  }finally{
