...
Write-Host '=== REINO TRIBAL - ENTRADA FINAL FIX5B ===' -ForegroundColor Cyan
Write-Host 'Turso DB response normalizada + Deno device auth sem keychain/TTY.' -ForegroundColor Green

@(
  (Join-Path $env:TEMP 'IMPLANTAR_REINO_TRIBAL_DENO_TURSO_FINAL.ps1'),
  (Join-Path $env:TEMP 'RT_REINO_TRIBAL_TURSO_DB_SHAPE_FIX5B_INNER.ps1'),
  $LauncherPath
...
$code=$LASTEXITCODE
if($code -ne 0){throw "Launcher FIX5B parou no proximo erro real. Codigo: $code"}
