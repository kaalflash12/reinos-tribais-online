...
$BasePath = Join-Path $env:TEMP 'RT_REINO_TRIBAL_TURSO_V2_BASE_FIX4B.ps1'
$BootstrapFinal = Join-Path $env:TEMP 'IMPLANTAR_REINO_TRIBAL_DENO_TURSO_FINAL.ps1'
$utf8Bom = New-Object Text.UTF8Encoding($true)

Write-Host '=== REINO TRIBAL FIX4B - DENO DEVICE AUTH SEM KEYCHAIN ===' -ForegroundColor Cyan
Write-Host 'Turso v2/v1 + Deno browser device flow + token apenas nesta execucao + zero indexacao de org.' -ForegroundColor Green
...
if(-not $texto.Contains($old)){ throw 'Ponto de autenticacao Deno antigo nao encontrado para FIX4B.' }
...
$unsafeOrg = "    if (-not `$denoOrg) { `$denoOrg = @(`$slugs | Sort-Object | Select-Object -First 1)[0] }"
$safeOrg = @'
...
'@
$safeOrg=$safeOrg.Replace("`r`n","`n").TrimEnd("`n")
if(-not $texto.Contains($unsafeOrg)){ throw 'Selecao Deno indexada esperada nao encontrada para FIX4B.' }
$texto=$texto.Replace($unsafeOrg,$safeOrg)

# Todas as chamadas Deno seguintes herdam DENO_DEPLOY_TOKEN e operam sem prompt.
...
foreach($needle in @(
  ...
  if(-not $texto.Contains($needle)){ throw "Contrato FIX4B ausente: $needle" }
}
foreach($forbidden in @(
  "Aviso 'Se o Deno ainda nao estiver autenticado, confirme somente o login oficial que abrir no navegador.'",
  '@($slugs | Sort-Object | Select-Object -First 1)[0]'
)){
  if($texto.Contains($forbidden)){ throw "Fluxo Deno antigo/indexado reapareceu: $forbidden" }
}
...
if($errors.Count){ throw ('Bootstrap FIX4B nao passou no parser: '+(($errors|ForEach-Object{$_.Message+' @ '+$_.Extent.StartLineNumber+':'+$_.Extent.StartColumnNumber}) -join "`n")) }
...
Write-Host 'PASS: DENO ORG = ZERO INDEXACAO [0].' -ForegroundColor Green
Write-Host 'PASS: TURSO = V2/V1 + ZERO INDEXACAO VAZIA MANTIDOS.' -ForegroundColor Green
Write-Host 'PASS: PARSER FINAL FIX4B VALIDADO.' -ForegroundColor Green

if($ValidateOnly){ Write-Host 'DENO_DEVICE_AUTH_FIX4B_VALIDATE_PASS' -ForegroundColor Green; return }

& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $BootstrapFinal
$code=$LASTEXITCODE
if($code -ne 0){ throw "FIX4B parou no proximo erro real. Codigo: $code" }
