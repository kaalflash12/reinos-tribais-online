...
$DenoExe = if ($DenoExeOverride) {
  $DenoExeOverride
} elseif (Test-Path $PortableDeno) {
  $PortableDeno
} else {
  $cmd = Get-Command deno.exe -ErrorAction SilentlyContinue
  if ($cmd) { $cmd.Source } else { $PortableDeno }
}
$HelperPath = Join-Path $PSScriptRoot 'rt91_process_helpers.ps1'
...
if (-not (Test-Path $HelperPath)) { throw "Helper nativo RT91 ausente: $HelperPath" }
. $HelperPath
...
function Add-OrgCandidates {
  ...
  if ($Node -is [System.Collections.IEnumerable] -and -not ($Node -is [pscustomobject])) {
    foreach ($item in $Node) {
      Add-OrgCandidates -Node $item -Set $Set
    }
    return
  }
  foreach ($prop in $Node.PSObject.Properties) {
    if (($prop.Name -eq 'slug' -or $prop.Name -eq 'name') -and $prop.Value -is [string]) {
      ...
      if ($v -match '^[A-Za-z0-9][A-Za-z0-9._-]{1,127}$') {
        [void]$Set.Add($v)
      }
    }
...
function Resolver-DenoOrgEApp {
  ...
  if (-not $who.Ok) {
    Write-Host 'Sessao Deno ausente ou expirada. Abrindo autorizacao oficial no navegador.' -ForegroundColor Yellow
    $loginCode = Invoke-RTDenoInteractive -DenoExe $DenoExe -CommandArgs @('deploy','env','list','--org',$PreferredDenoOrg,'--app',$DenoApp)
    Write-Host ('Retorno do gatilho de autenticacao: ' + $loginCode) -ForegroundColor DarkGray
    $who = Invoke-RTDenoJson -DenoExe $DenoExe -CommandArgs @('deploy','whoami')
  }
  if (-not $who.Ok) {
    Falhar ('Deno whoami nao confirmou sessao autenticada. ' + $who.Text)
  }
  ...
  if (-not [string]::IsNullOrWhiteSpace($PreferredDenoOrg)) {
    [void]$orgSet.Add($PreferredDenoOrg)
  }
  if ($orgSet.Count -eq 0) {
    Falhar 'Conta Deno autenticada sem organizacoes detectaveis. Nenhum secret foi alterado.'
  }
  ...
  foreach ($candidate in @($orgSet)) {
    ...
    if ($probe.Ok) {
      ...
      if ($serialized.Contains($ExpectedHost)) {
        $resolved = $candidate
        break
      }
    }
  ...
  if (-not $envCheck.Ok) {
    Falhar ('App canonico localizado em ' + $resolved + ', mas env list foi recusado. ' + $envCheck.Text)
  }
...
function Invoke-HttpJson {
  ...
  try {
    $result = Invoke-WebRequest -UseBasicParsing -Method Post -Uri $Url -ContentType 'application/json' -Headers $headers -Body $Json -TimeoutSec $TimeoutSec
...
function Parse-HttpJson {
  ...
  if (-not $Result.Ok) {
    Falhar ("$Label falhou HTTP $($Result.Status). $($Result.Text)")
  }
  try {
...
function Gravar-Pendente {
  param([string]$Password,[string]$Recovery,[string]$Status)
...
Write-Host ''
Write-Host '=== REINO TRIBAL EXECUTOR FINAL RT91 AUTH10 ===' -ForegroundColor Cyan
...
if (-not (Test-Path $DenoExe)) {
  Falhar "Deno $DenoVersion nao encontrado: $DenoExe"
}
$ver = Invoke-RTDenoText -DenoExe $DenoExe -CommandArgs @('--version')
if ($ver.Code -ne 0 -or $ver.Stdout -notmatch "deno $([regex]::Escape($DenoVersion))") {
  Falhar ('Versao Deno inesperada. stdout=' + $ver.Stdout + ' stderr=' + $ver.Stderr)
}
...
if ($DiscoveryTestOnly) {
  if ($env:RT91_FAKE_DENO_SELFTEST -eq '1') {
    $testUpdate = Invoke-RTDenoJson -DenoExe $DenoExe -CommandArgs @('deploy','env','update-value','RT_SELFTEST_ONLY','sentinel','--org',$ResolvedDenoOrg,'--app',$DenoApp,'--non-interactive')
    if (-not $testUpdate.Ok) {
      Falhar ('Selftest update-value falhou. ' + $testUpdate.Text)
    }
    Ok 'DENO_UPDATE_VALUE_STDERR_SELFTEST_PASS'
  }
  Ok 'RT91_AUTH10_DISCOVERY_TEST_PASS'
  return
}

Etapa 'Validando backend Turso antes de alterar secrets'
$healthResult = Invoke-HttpJson -Url ($ExpectedBackend + '/api/reino') -Json '{"action":"health"}'
$health = Parse-HttpJson -Result $healthResult -Label 'health'
if (-not $health.ok -or $health.database -ne 'turso') {
  Falhar 'Health nao confirmou Turso. Nenhum secret foi alterado.'
}
...
$up1 = Invoke-RTDenoJson -DenoExe $DenoExe -CommandArgs @('deploy','env','update-value','RT_ADMIN_PASSWORD',$adminPassword,'--org',$ResolvedDenoOrg,'--app',$DenoApp,'--non-interactive')
if (-not $up1.Ok) {
  Falhar ('RT_ADMIN_PASSWORD recusado. ' + $up1.Text)
}
Gravar-Pendente -Password $adminPassword -Recovery $recoveryKey -Status 'RT_ADMIN_PASSWORD_ATUALIZADO'

$up2 = Invoke-RTDenoJson -DenoExe $DenoExe -CommandArgs @('deploy','env','update-value','RT_ADMIN_RECOVERY_KEY',$recoveryKey,'--org',$ResolvedDenoOrg,'--app',$DenoApp,'--non-interactive')
if (-not $up2.Ok) {
  Falhar ('RT_ADMIN_RECOVERY_KEY recusado. ' + $up2.Text)
}
...
for ($i=1; $i -le 30; $i++) {
  ...
  if ($r.Ok) {
    $candidate = $null
    try {
      $candidate = $r.Text | ConvertFrom-Json
    } catch {
      $candidate = $null
    }
...
if (-not $login -or -not [string]$login.access_token) {
  Falhar 'Nova credencial nao ficou ativa; arquivo PENDENTES foi preservado.'
}
if ([string]$login.user.username -ne 'reinos_admin') {
  Falhar 'Login retornou usuario diferente de reinos_admin.'
}
...
if (-not $status.ok) {
  Falhar 'admin_status nao confirmou ok.'
}
...
if ($null -eq $dash) {
  Falhar 'dashboard nao retornou dados.'
}
