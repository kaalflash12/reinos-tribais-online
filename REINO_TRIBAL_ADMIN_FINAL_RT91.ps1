...
$DenoExe = ''
if ($DenoExeOverride) {
  $DenoExe = $DenoExeOverride
} elseif (Test-Path $PortableDeno) {
  $DenoExe = $PortableDeno
} else {
  $cmd = Get-Command deno.exe -ErrorAction SilentlyContinue
  if ($cmd) { $DenoExe = $cmd.Source } else { $DenoExe = $PortableDeno }
}
...
function Invoke-RTDenoJson {
  ...
  if ([string]::IsNullOrWhiteSpace($r.Stdout)) {
    return [pscustomobject]@{ Ok = $false; Code = 0; Data = $null; Stdout = ''; Stderr = $r.Stderr; Text = 'Comando retornou stdout JSON vazio.' }
  }
  try {
    $data = $r.Stdout | ConvertFrom-Json
  } catch {
    return [pscustomobject]@{ Ok = $false; Code = 0; Data = $null; Stdout = $r.Stdout; Stderr = $r.Stderr; Text = ('JSON invalido em stdout. ' + $r.Stdout) }
  }
