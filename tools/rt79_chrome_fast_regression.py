...
def e2e(d):
 js(d,"const s=RT76.state(),v=s.villages[s.activeVillageId];v.resources.wood=v.resources.clay=v.resources.iron=999999;v.buildQueue=[];v.recruitQueue=[];v.units.spear=500;for(const k of Object.keys(v.buildings||{})){const mx=Number(window.GAME_DATA?.buildings?.[k]?.max||30);v.buildings[k]=Math.min(mx,Math.max(Number(v.buildings[k]||0),['farm','warehouse'].includes(k)?30:10));}s.research=s.research||{queue:[],completed:[]};s.research.queue=[];if(v.unitResearch)v.unitResearch.spear=1;RT76.save()")
