...
def click(d,sel):
 rec('click '+sel,js(d,"const e=document.querySelector(arguments[0]);if(!e)return false;e.click();return true",sel));time.sleep(.08)
def nav(d,suffix=''):
 try:d.get(URL+suffix)
 except TimeoutException:pass
 WebDriverWait(d,12).until(lambda x: js(x,'return !!document.body'))
 WebDriverWait(d,12).until(lambda x: js(x,'return !!window.__RT79_STRATEGY_SUITE__'))

def start(d):
 nav(d,'?fast=1');js(d,"Object.keys(localStorage).filter(k=>k.startsWith('reinos_tribais_ptbr_save_')).forEach(k=>localStorage.removeItem(k))")
 ...
 WebDriverWait(d,10).until(lambda x:js(x,'return !!window.RT76?.state?.()?.activeVillageId'));rec('game started',True)
...
def e2e(d):
 js(d,"const s=RT76.state(),v=s.villages[s.activeVillageId];v.resources.wood=v.resources.clay=v.resources.iron=999999;v.buildQueue=[];v.recruitQueue=[];v.units.spear=500;for(const k of Object.keys(v.buildings||{}))v.buildings[k]=Math.max(Number(v.buildings[k]||0),['farm','warehouse'].includes(k)?20:10);s.research=s.research||{queue:[],completed:[]};s.research.queue=[];if(v.unitResearch)v.unitResearch.spear=1;RT76.save()")
 ...
 click(d,'[data-view="research"]');before=js(d,"return (RT76.state().research?.queue||[]).length");key=js(d,"const e=document.querySelector('[data-research]:not([disabled])');if(!e)return '';e.click();return e.dataset.research||''");after=js(d,"return (RT76.state().research?.queue||[]).length");rec('research queue',bool(key) and after>before,key)
...
def online_mock(d):
 ...
 js(d,'window.RT79.open()');WebDriverWait(d,6).until(lambda x:js(x,"return document.querySelector('#rt79-overlay')?.classList.contains('open')"))
 ...
 js(d,'window.RT79.open()');time.sleep(.1);click(d,'[data-rt79-tab="market"]');WebDriverWait(d,5).until(lambda x:js(x,"return !!document.querySelector('#rt79-logistics-card')"));rec('logistics UI',True);click(d,'[data-rt79-tab="farm"]');WebDriverWait(d,5).until(lambda x:js(x,"return !!document.querySelector('#rt79-barb-ai-card')"));rec('barbarian AI UI',True);click(d,'[data-rt79-tab="manager"]');WebDriverWait(d,5).until(lambda x:js(x,"return !!document.querySelector('#rt79-group-goals-card')"));rec('group goals UI',True)

def main():
 opts=Options();opts.page_load_strategy='none';opts.add_argument('--headless=new');opts.add_argument('--no-sandbox');opts.add_argument('--disable-gpu');opts.add_argument('--disable-dev-shm-usage');opts.add_argument('--window-size=1280,800');opts.add_experimental_option('prefs',{'profile.managed_default_content_settings.images':2})
 d=webdriver.Chrome(options=opts);d.set_page_load_timeout(5);d.set_script_timeout(5)
 proof={}
 try:
  start(d);base_views(d);building_gate(d);e2e(d);online_mock(d);d.set_window_size(390,844);rec('mobile suite',js(d,'return !!window.__RT79_STRATEGY_SUITE__'));proof={'pass':True,'browser':'chrome','tests':len(M),'manifest':M}
 except Exception as e:
  proof={'pass':False,'browser':'chrome','error':repr(e),'tests':len(M),'manifest':M};raise
 finally:
