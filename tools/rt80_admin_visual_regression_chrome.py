...
def shot(d,name):
    p=OUT/f'{len(proof["screenshots"])+1:02d}_{name}.png'
    d.save_screenshot(str(p));proof['screenshots'].append(p.name);save_proof();log('SHOT '+name)
def close_modals(d):
...
def open_modal(d,selector,name):
    ...
    d.execute_script("window.RT80AdminVisual?.refresh?.()")
...
def prepare():
    ...
    # Audit-only: stop legacy/global MutationObservers from feeding each other while
    # the real admin renderer mounts its DOM. The RT80 visual runtime is applied
    # explicitly after the shell exists.
    guard="""<script id=\"rt80-admin-audit-observer-guard\">\nwindow.__RT80_REAL_MUTATION_OBSERVER__=window.MutationObserver;\nwindow.MutationObserver=class RT80AuditMutationObserver{constructor(cb){this.cb=cb}observe(){}disconnect(){}takeRecords(){return[]}};\n</script>\n"""
...
def main():
    ...
    for a in ['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--window-size=1600,1000']:
        opts.add_argument(a)
    opts.add_experimental_option('prefs',{'profile.managed_default_content_settings.images':2})
    opts.set_capability('goog:loggingPrefs',{'browser':'ALL'})
    d=None
    try:
        phase('creating_chrome_session');d=webdriver.Chrome(options=opts);phase('chrome_session_ready')
        d.set_page_load_timeout(30);d.set_script_timeout(12)
        ...
        # Dispatch on next task so Selenium gets control back before render starts.
        d.execute_script("""
          const data=arguments[0],a=window.__RT80_ADMIN_AUDIT__;
          ...
          window.__RT80_ADMIN_RENDER_RESULT__={started:false,settled:false,error:null};
          setTimeout(()=>{window.__RT80_ADMIN_RENDER_RESULT__.started=true;try{Promise.resolve(a.renderIntegratedAdmin(data,true)).then(()=>window.__RT80_ADMIN_RENDER_RESULT__.settled=true).catch(e=>{window.__RT80_ADMIN_RENDER_RESULT__.settled=true;window.__RT80_ADMIN_RENDER_RESULT__.error=String(e?.stack||e)})}catch(e){window.__RT80_ADMIN_RENDER_RESULT__.settled=true;window.__RT80_ADMIN_RENDER_RESULT__.error=String(e?.stack||e)}},0);
          return true;
        """,base.MOCK)
        ...
        err=d.execute_script("return window.__RT80_ADMIN_RENDER_RESULT__?.error||null")
        if err: raise RuntimeError(err)
        phase('admin_dom_ready')
        # Apply the real RT80 admin visual runtime once to the already-rendered DOM.
        d.execute_script("eval(arguments[0]);",RUNTIME)
        d.execute_script("window.RT80AdminVisual?.refresh?.()")
        phase('visual_runtime_applied')
        ...
        d.set_window_size(430,932);time.sleep(.1);d.execute_script("document.querySelector('[data-admin-tab=\"overview\"]')?.click();window.RT80AdminVisual?.refresh?.()");time.sleep(.05)
        ...
        proof['rendererPromiseSettled']=bool(d.execute_script("return window.__RT80_ADMIN_RENDER_RESULT__?.settled===true"))
