...
preview=text('supabase/functions/rt-multiplayer-v82/index.ts')
...
must('browser promoted to RT82 authority endpoint',"edgePath: '/functions/v1/rt-multiplayer-v82'" in html)
must('RT82 client uses metadata-only village sync','startProfile: state?.settings?.startProfile || null' in html and 'rt82VillageRpc' in html)
must('RT82 client routes manual building/recruit/research actions to RPCs',all(x in html for x in ['rt82_queue_building','rt82_queue_recruit','rt82_queue_unit_research','rt82_queue_global_research']))
must('RT82 preview sync is metadata-only','async function syncVillage' in preview and 'name:cleanName' in preview and 'v.resources' not in preview[preview.find('async function syncVillage'):preview.find('function publicBuildings')])
must('v60 router targets RT81 root','/functions/v1/rt-multiplayer`' in router and '/functions/v1/rt-multiplayer-v59' not in router)
must('v60 router revision explicit',"router_revision='rt81.4'" in router and 'data.version=60' in router)
