...
$ExecutorRevision = 'SAFE-CLI-AUTH-4'
...
$Tmp = Join-Path $env:TEMP ('rt91-cli-auth4-' + [Guid]::NewGuid().ToString('N'))
$BasePath = Join-Path $Tmp 'RT91_BASE.ps1'
$PatchedPath = Join-Path $Tmp 'RT91_CLI_AUTH4_PATCHED.ps1'
...
try {
  ...
  Write-Host ''
  Write-Host '=== REINO TRIBAL RT91 CLI-AUTH-4 ===' -ForegroundColor Cyan
  ...
  # 2) Autenticacao: usar apenas comando documentado e read-only do Deno Deploy.
  #    `deno deploy env list --org ... --app ...` aciona a autenticacao automatica do CLI
  #    quando necessario e usa o keyring do sistema. Nenhum endpoint /auth e chamado por este script.
...
function Garantir-DenoAuth {
  ...
  Executar-Deno-Interativo -Args @('deploy','env','list','--org',$DenoOrg,'--app',$DenoApp) -TimeoutSec 420 -Rotulo 'Deno CLI env list + navegador'
...
'@
  ...
  $text = $text.Replace("`$ExecutorRevision = 'SAFE-CLI-AUTH-2'","`$ExecutorRevision = 'SAFE-CLI-AUTH-4'")
  ...
  if ($errors.Count) { $errors|Format-List|Out-String|Write-Host; throw 'RT91 CLI-AUTH-4 nao passou no parser PowerShell.' }
  ...
  foreach ($needle in @(
    ...
    'ANTI_DOWNGRADE: nenhum source deploy executado = PASS',
    'REINO_TRIBAL_ADMIN_RT91_VALIDADO'
  )) { if (-not $patched.Contains($needle)) { throw "Patch RT91 CLI-AUTH-4 incompleto: $needle" } }
  ...
  Write-Host 'PASS: RT91 CLI-AUTH-4 parseado.' -ForegroundColor Green
  ...
  & powershell.exe @args
  $code=$LASTEXITCODE
  if($code -ne 0){throw "RT91 CLI-AUTH-4 terminou com codigo $code"}
} finally {
