...
try{
  ...
  Write-Host '=== REINOS TRIBAIS - FECHAMENTO ADM PUBLICO ===' -ForegroundColor Cyan
  ...
  Pass 'launcher FIX17 baixado e parseado'

  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $Launcher
  if($LASTEXITCODE -ne 0){Fail ('FIX17 terminou com exit code '+$LASTEXITCODE)}
  ...
  Pass 'credencial FIX17 validada localmente sem imprimir senha'

  $Deno=Join-Path (Join-Path $env:LOCALAPPDATA 'ReinoTribalTools\deno-2.9.5') 'deno.exe'
  if(-not (Test-Path $Deno)){$dc=Get-Command deno.exe -ErrorAction SilentlyContinue;if($dc){$Deno=$dc.Source}else{Fail 'Deno 2.9.5 não encontrado após FIX17.'}}
