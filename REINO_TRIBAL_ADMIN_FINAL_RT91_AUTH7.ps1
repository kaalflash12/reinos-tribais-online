...
$WorkRoot = Join-Path $env:TEMP ('reino-tribal-admin-final-auth7-' + [Guid]::NewGuid().ToString('N'))
...
function Invoke-DenoJson {
  ...
  try{
    if($QuietErrors){$raw=@(& $DenoExe @CommandArgs --json 2>$errFile)}else{$raw=@(& $DenoExe @CommandArgs --json)}
    $code=$LASTEXITCODE
    ...
    if([string]::IsNullOrWhiteSpace($stdout)){return [pscustomobject]@{Ok=$false;Code=1;Text='JSON vazio';Data=$null}}
    try{$data=$stdout|ConvertFrom-Json}catch{return [pscustomobject]@{Ok=$false;Code=1;Text=('JSON invalido: '+$stdout);Data=$null}}
    return [pscustomobject]@{Ok=$true;Code=0;Text=$stdout;Data=$data}
...
function Curl-Json {
  ...
  try{
    ...
    for($i=1;$i -le $Attempts;$i++){
      ...
      $http=((& $CurlCmd.Source @a)|Out-String).Trim()
      $code=$LASTEXITCODE
...
function Get-GitHubJson([string]$Url){
  ...
  try{
    & $CurlCmd.Source --fail --silent --show-error --location --http1.1 --tlsv1.2 --retry 4 --retry-all-errors --connect-timeout 20 --max-time 90 -H 'Accept: application/vnd.github+json' -H 'User-Agent: ReinoTribal-RT91' --output $out $Url
    if($LASTEXITCODE -ne 0){Falhar 'Falha consultando GitHub.'}
...
function Baixar-E-Validar-MainAtual([string]$Destino){
  ...
  foreach($relative in @('deno.json','package.json','deno/main.js','api/reino.js','api/admin.js','api/realtime.js','backend/turso/schema.sql')){
    ...
    & $CurlCmd.Source --fail --silent --show-error --location --http1.1 --tlsv1.2 --retry 4 --retry-all-errors --connect-timeout 20 --max-time 120 --output $dest $url
    if($LASTEXITCODE -ne 0){Falhar "Falha baixando main atual: $relative"}
  }
  ...
  try{& $DenoExe check deno/main.js;$checkCode=$LASTEXITCODE}finally{Pop-Location}
  if($checkCode -ne 0){Falhar "Backend atual nao passou no deno check. exit=$checkCode"}
...
function Testar-Preflight {
  ...
  try{
    $http=((& $CurlCmd.Source --silent --show-error --http1.1 --tlsv1.2 --connect-timeout 20 --max-time 30 -X OPTIONS -D $headers -o NUL -H 'Origin: https://kaalflash12.github.io' -H 'Access-Control-Request-Method: POST' -H 'Access-Control-Request-Headers: content-type,authorization' --write-out '%{http_code}' ($ExpectedBackend+'/api/reino'))|Out-String).Trim()
    if($LASTEXITCODE -ne 0 -or $http -ne '204'){Falhar "CORS preflight nao retornou 204. http=$http"}
...
Write-Host ''
Write-Host '=== REINO TRIBAL EXECUTOR FINAL RT91 AUTH7 ===' -ForegroundColor Cyan
...
$ver=((& $DenoExe --version)|Out-String)
if($LASTEXITCODE -ne 0 -or $ver -notmatch "deno $([regex]::Escape($DenoVersion))"){Falhar ('Versao Deno inesperada. '+$ver)}
