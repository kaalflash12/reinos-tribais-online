...
$WorkRoot = Join-Path $env:TEMP ('reino-tribal-admin-fix15-' + [Guid]::NewGuid().ToString('N'))
...
function Curl-Json {
  ...
  $reqFile = Join-Path $env:TEMP ("rt-fix15-$id-request.json")
  $respFile = Join-Path $env:TEMP ("rt-fix15-$id-response.json")
...
function Testar-Preflight {
  $headers = Join-Path $env:TEMP ('rt-fix15-cors-' + [Guid]::NewGuid().ToString('N') + '.txt')
  try {
    $r = Executar-Nativo -Exe $CurlCmd.Source -Args @(
...
try {
  Etapa 'FIX15 - sincronizacao definitiva do administrador'
  ...
  if ($ValidateOnly) {
    Ok 'FIX15 ValidateOnly concluido; nenhum secret e nenhum deploy foram alterados.'
    return
  }
  ...
  Write-Host "`nREINO_TRIBAL_ADMIN_FIX15_VALIDADO" -ForegroundColor Green
