...
function Curl-Json {
  ...
  $reqFile = Join-Path $env:TEMP ("rt-fix16-$id-request.json")
  $respFile = Join-Path $env:TEMP ("rt-fix16-$id-response.json")
...
try {
  ...
  Write-Host "`nREINO_TRIBAL_ADMIN_FIX16_VALIDADO" -ForegroundColor Green
