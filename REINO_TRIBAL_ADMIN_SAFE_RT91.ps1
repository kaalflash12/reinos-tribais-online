...
$WorkRoot = Join-Path $env:TEMP ('reino-tribal-admin-rt91-' + [Guid]::NewGuid().ToString('N'))
...
$ExecutorRevision = 'SAFE-NO-DOWNGRADE-1'
...
function Curl-Json {
  ...
  try {
    ...
    for ($attempt=1; $attempt -le $Attempts; $attempt++) {
      ...
      if ($r.Code -eq 0 -and $status -gt 0) { return [pscustomobject]@{ Ok=($status -ge 200 -and $status -lt 300); Status=$status; Text=[string]$bodyText } }
...
function Get-JsonUrl([string]$Url) {
  ...
  try {
    ...
    try { return ([IO.File]::ReadAllText($out) | ConvertFrom-Json) } catch { Falhar 'GitHub retornou JSON invalido.' }
...
function Baixar-E-Validar-MainAtual([string]$Destino) {
  ...
  foreach ($needle in @("import realtimeHandler from '../api/realtime.js';","pathname === '/ws'","pathname === '/api/realtime/ws'")) {
    if (-not $denoMain.Contains($needle)) { Falhar "ANTI-DOWNGRADE: main atual sem contrato realtime: $needle" }
  }
  $reino = [IO.File]::ReadAllText((Join-Path $Destino 'api\reino.js'))
  foreach ($needle in @('const passwordMatches = verifyPassword(password, existing.password_hash);',"role='admin',disabled=0",'DELETE FROM rt_sessions WHERE user_id=?','normalizeUsername(identifier) === ADMIN_USERNAME) await ensureAdmin();')) {
    if (-not $reino.Contains($needle)) { Falhar "main atual sem contrato ADM: $needle" }
  }
...
function Testar-Preflight {
  $preflightUrl = $Backend.TrimEnd('/') + '/api/reino'
  $headers = Join-Path $env:TEMP ('rt91-cors-' + [Guid]::NewGuid().ToString('N') + '.txt')
  try {
    $r = Executar-Nativo -Exe $CurlCmd.Source -Args @('--silent','--show-error','--http1.1','--tlsv1.2','--connect-timeout','20','--max-time','30','--output','NUL','--dump-header',$headers,'--request','OPTIONS','-H','Origin: https://kaalflash12.github.io','-H','Access-Control-Request-Method: POST','-H','Access-Control-Request-Headers: content-type,authorization',$preflightUrl) -TimeoutSec 45 -Rotulo 'CORS preflight publico'
    ...
    if ($h -notmatch '(?im)^access-control-allow-origin:\s*https://kaalflash12\.github\.io\s*$') { Falhar "CORS allow-origin nao autoriza GitHub Pages.`n$h" }
...
function Gravar-CheckpointCredencial {
  param([Parameter(Mandatory=$true)][string]$Password,[Parameter(Mandatory=$true)][string]$Recovery,[Parameter(Mandatory=$true)][string]$Status)
...
Write-Host ''
Write-Host '=== REINO TRIBAL EXECUTOR SEGURO RT91 ===' -ForegroundColor Cyan
...
$markerText = @('version='+$ExecutorVersion,'revision='+$ExecutorRevision,'contract='+$ExecutorContract,'backend='+$Backend,'started_utc='+[DateTime]::UtcNow.ToString('o')) -join [Environment]::NewLine
...
try {
  ...
  if ($CheckpointFailureTestOnly) {
    Gravar-CheckpointCredencial -Password 'RT!CHECKPOINT_FAILURE_TEST_1234567890' -Recovery 'CHECKPOINT_FAILURE_TEST_RECOVERY_1234567890' -Status 'CI_FALHA_SIMULADA_ANTES_DO_DENO'
  ...
  Testar-Preflight
  Ok 'Producao atual e CORS passaram antes da troca.'
  ...
  $login = $null
  for ($i=1; $i -le 30; $i++) {
    $healthResult = Curl-Json -Url ($Backend + '/api/reino') -Json '{"action":"health"}' -Attempts 1
    if ($healthResult.Ok) {
      $loginJson = @{ action='login'; identifier='reinos_admin'; password=$adminPassword } | ConvertTo-Json -Compress
      $loginResult = Curl-Json -Url ($Backend + '/api/reino') -Json $loginJson -Attempts 1
      if ($loginResult.Ok) {
        try { $candidate = $loginResult.Text | ConvertFrom-Json } catch { $candidate = $null }
        if ($candidate -and [string]$candidate.access_token) { $login = $candidate; break }
      }
    }
    Start-Sleep -Seconds 3
  }
  if (-not $login -or -not [string]$login.access_token) { Falhar 'Nova credencial nao ficou ativa; nenhuma versao antiga foi redeployada. Use o arquivo PENDENTES para recuperacao.' }
  ...
  Testar-Preflight
  Ok 'login + admin_status + dashboard + CORS passaram.'
  ...
  if (Test-Path $PendingCredFile) {
    ...
    Write-Host 'Nao use como validada ate login + admin_status + dashboard passarem.' -ForegroundColor Yellow
  }
  throw
} finally {
  $env:DENO_DEPLOY_TOKEN = ''
