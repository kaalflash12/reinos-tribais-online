...
$ExecutorRevision='SAFE-CLI-AUTH-9-FINAL'
...
function Resolver-DenoOrgEApp {
  ...
  if(-not $who.Ok){
    ...
    # Este comando e apenas um gatilho de login. Exit 4 (org legado nao visivel) e aceitavel depois da autenticacao.
...
Write-Host ''
Write-Host '=== REINO TRIBAL EXECUTOR FINAL RT91 AUTH9 ===' -ForegroundColor Cyan
...
if($DiscoveryTestOnly){
  if($env:RT91_FAKE_DENO_SELFTEST -eq '1'){
    ...
    if(-not $testUpdate.Ok){Falhar ('Selftest update-value falhou. '+$testUpdate.Text)}
    Ok 'DENO_UPDATE_VALUE_STDERR_SELFTEST_PASS'
  }
  Ok 'RT91_AUTH9_DISCOVERY_TEST_PASS'
  return
}
...
if(-not $up1.Ok){Falhar ('RT_ADMIN_PASSWORD recusado. '+$up1.Text)}
...
if(-not $up2.Ok){Falhar ('RT_ADMIN_RECOVERY_KEY recusado. '+$up2.Text)}
