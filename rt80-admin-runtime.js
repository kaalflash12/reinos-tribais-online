...
(()=>{
  ...
  function normalizeText(root){
    ...
    nodes.forEach(n=>{n.nodeValue=(n.nodeValue||'').replace(/Versão\s+79\b/gi,'Versão RT80').replace(/\bRT(?:[1-7]\d)(?:\.\d+)?\b/gi,'RT80')});
  }

  function normalizeHeader(shell){
    const top=$('.rt60-admin-top',shell);if(!top)return;
    const h=$('h1',top);if(h)h.textContent=(h.textContent||'REINOS TRIBAIS • CENTRAL OPERACIONAL').replace(/\bRT(?:[1-7]\d)(?:\.\d+)?\b/gi,'RT80');
    const p=$('p',top);if(p){
      let t=p.textContent||'';
      t=t.replace(/backend\s+v\d+(?:\.\d+)?/i,'backend online');
      t=t.replace(/interface\s+guiada\s+RT\d+(?:\.\d+)?/i,'interface guiada RT80');
      t=t.replace(/\bRT(?:[1-7]\d)(?:\.\d+)?\b/gi,'RT80');
      p.textContent=t;
    }
  }
  ...
  function accessibility(shell){
    ...
    $$('.rt64-modal,.rt60-admin-modal').forEach(modal=>{modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');const title=$('h2',modal);if(title&&!title.id){title.id='rt80-admin-dialog-'+Math.random().toString(36).slice(2,9);modal.setAttribute('aria-labelledby',title.id)}});
  }

  function markEmptyStates(shell){
    $$('.rt60-admin-empty',shell).forEach(el=>{el.classList.add('rt80-admin-empty');if(!el.dataset.rt80Enhanced){el.dataset.rt80Enhanced='1';el.setAttribute('role','status')}});
    $$('td[colspan]',shell).forEach(td=>{const t=(td.textContent||'').trim();if(/^(sem|nenhum|nenhuma)/i.test(t)){td.classList.add('rt80-admin-empty-cell');td.setAttribute('role','status')}});
  }
  ...
  function ensure(){
    ...
    if(shell){
      ...
      shell.dataset.rt80AdminReady='1';document.body.classList.add('rt80-admin-mode');
    }else document.body.classList.remove('rt80-admin-mode');
    $$('.rt64-modal,.rt60-admin-modal').forEach(m=>{normalizeText(m);accessibility(document);m.dataset.rt80AdminModal='1'});
  }
